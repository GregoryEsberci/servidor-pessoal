# Índice de Documentação

- [Configurações iniciais](https://github.com/GregoryEsberci/servidor-pessoal/blob/main/docs/configuracoes-iniciais.md)
- [Configurar Docker e Docker Compose v2](https://github.com/GregoryEsberci/servidor-pessoal/blob/main/docs/configurar-docker-compose.md)
- [Configurar Git](https://github.com/GregoryEsberci/servidor-pessoal/blob/main/docs/configurar-git.md)
- [Configurar HD secundário](https://github.com/GregoryEsberci/servidor-pessoal/blob/main/docs/configurar-hd-secundario.md)
- [Configurar SFTP nextcloud](https://github.com/GregoryEsberci/servidor-pessoal/blob/main/docs/configurar-sftp-nextcloud.md)
- [Configurar SSH](https://github.com/GregoryEsberci/servidor-pessoal/blob/main/docs/configurar-ssh.md)
- [Evitar suspensão ao fechar a tampa do laptop](https://github.com/GregoryEsberci/servidor-pessoal/blob/main/docs/configurar-tampa-laptop.md)
- [Limitar logs dos containers](https://github.com/GregoryEsberci/servidor-pessoal/blob/main/docs/limitar-logs-containers.md)
